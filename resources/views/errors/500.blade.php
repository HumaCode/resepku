...
<head>
  ...
  <title>500 — Kesalahan Server — {{ config('app.name', 'ResepKita') }}</title>
...
<body>
  ...
  <!-- Blobs -->
  <div class="blob" style="width:420px;height:420px;background:rgba(239,68,68,.08);top:-120px;right:-100px"></div>
  <div class="blob" style="width:300px;height:300px;background:rgba(232,93,38,.07);bottom:-80px;left:-80px"></div>
  ...
  <div class="error-wrap">
    ...
    <!-- Card -->
    <div class="error-card" style="--stripe:linear-gradient(90deg,#f87171,#ef4444)">
      <style>.error-card::before{background:linear-gradient(90deg,#f87171,#ef4444)}</style>

      <span class="error-emoji">💥</span>
      <span class="error-code">500</span>
      <span class="error-pill" style="background:rgba(239,68,68,.09);border:1.5px solid rgba(239,68,68,.25);color:#dc2626"><i class="bi bi-server"></i> Internal Server Error — 500</span>
      <h1 class="error-title">Ada yang Salah di Dapur Kami</h1>
      <p class="error-desc">{{ $exception->getMessage() ?: 'Server kami sedang mengalami gangguan internal. Tim teknis sudah mendapat notifikasi otomatis dan sedang bekerja keras memperbaikinya.' }}</p>

      @php
        $errId = 'ERR-500-RK-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -4));
      @endphp
      <div class="err-id-box" onclick="copyErrId(this)" data-id="{{ $errId }}" title="Klik untuk menyalin">
        <i class="bi bi-clipboard"></i>
        <span id="errIdVal">{{ $errId }}</span>
        <span id="errIdTime" style="color:var(--muted);font-size:.72rem"></span>
      </div>
      <div class="error-actions">
        <button class="btn-primary-e" onclick="window.location.reload()"><i class="bi bi-arrow-clockwise"></i> Coba Lagi</button>
        <a href="{{ auth()->check() ? route('dashboard') : url('/') }}" class="btn-outline-e"><i class="bi bi-house-fill"></i> Ke Dashboard</a>
      </div>
      <div class="error-tips">
        <div class="error-tips-ttl"><i class="bi bi-bug"></i> Informasi untuk Tim Teknis</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Tim teknis telah mendapat notifikasi otomatis</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Coba lagi dalam beberapa menit — server biasanya pulih sendiri</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Salin Error ID di atas untuk dilaporkan ke <a href="#" class="tip-link">Support</a></div>
        <div class="tip-item"><i class="bi bi-check2"></i> Jika darurat, hubungi admin melalui <a href="#" class="tip-link">WhatsApp Support</a></div>
      </div>
  ...
  <script>
    /* Timestamp */
    const now = new Date();
    document.getElementById('errIdTime').textContent =
      '· ' + now.toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'}) + ' WIB';
    /* Copy Error ID */
    function copyErrId(box) {
      const errId = box.getAttribute('data-id');
      navigator.clipboard.writeText(errId).then(() => {
