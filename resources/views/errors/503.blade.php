...
<head>
  ...
  <title>503 — Layanan Tidak Tersedia — {{ config('app.name', 'ResepKita') }}</title>
...
<body>
  ...
  <!-- Blobs -->
  <div class="blob" style="width:420px;height:420px;background:rgba(59,130,246,.08);top:-120px;right:-100px"></div>
  <div class="blob" style="width:290px;height:290px;background:rgba(232,93,38,.06);bottom:-70px;left:-70px"></div>
  ...
  <div class="error-wrap">
    ...
    <!-- Card -->
    <div class="error-card" style="--stripe:linear-gradient(90deg,#60a5fa,#3b82f6)">
      <style>.error-card::before{background:linear-gradient(90deg,#60a5fa,#3b82f6)}</style>

      <span class="error-emoji">🔧</span>
      <span class="error-code">503</span>
      <span class="error-pill" style="background:rgba(59,130,246,.09);border:1.5px solid rgba(59,130,246,.25);color:#1d4ed8"><i class="bi bi-tools"></i> Service Unavailable — 503</span>
      <h1 class="error-title">Sedang dalam Pemeliharaan</h1>
      <p class="error-desc">{{ $exception->getMessage() ?: 'ResepKita sedang dalam proses pemeliharaan terjadwal. Kami sedang menyempurnakan sistem agar pengalamanmu semakin nikmat. Mohon bersabar sebentar!' }}</p>

      
      <div style="margin-bottom:.25rem">
        <div class="countdown-val" id="cd503">02:00:00</div>
        <div class="countdown-lbl">Estimasi selesai pemeliharaan</div>
        <div class="prog-track"><div class="prog-fill"></div></div>
        <div style="font-size:.72rem;color:var(--muted);margin-bottom:1rem">Proses pemeliharaan sedang berjalan...</div>
      </div>
      <div class="status-chips">
        <span class="s-chip chip-ok"><i class="bi bi-check-circle-fill"></i> Database OK</span>
        <span class="s-chip chip-busy"><i class="bi bi-wrench-adjustable"></i> API Maintenance</span>
        <span class="s-chip chip-ok"><i class="bi bi-check-circle-fill"></i> CDN OK</span>
        <span class="s-chip chip-busy"><i class="bi bi-wrench-adjustable"></i> Cache Rebuild</span>
      </div>
      <div class="error-actions">
        <button class="btn-primary-e" onclick="window.location.reload()"><i class="bi bi-arrow-clockwise"></i> Periksa Lagi</button>
        <button class="btn-outline-e" id="notifBtn" onclick="subscribeNotif()"><i class="bi bi-bell"></i> Beritahu Saya</button>
      </div>
      <div class="error-tips">
        <div class="error-tips-ttl"><i class="bi bi-calendar-check"></i> Jadwal Pemeliharaan</div>
        <div class="tip-item"><i class="bi bi-clock"></i> Mulai: <strong style="color:var(--secondary)">{{ date('l, d F Y') }} — 02.00 WIB</strong></div>
        <div class="tip-item"><i class="bi bi-clock-history"></i> Estimasi selesai: <strong style="color:var(--secondary)">04.00 WIB</strong> (~2 jam)</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Semua data aman dan tidak ada yang hilang</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Halaman ini akan refresh otomatis setiap 60 detik</div>
      </div>
  ...
  <script>
    /* Countdown 503 — 2 jam */
    let s503 = 7200;
    const el503 = document.getElementById('cd503');
    setInterval(() => {
      if (s503 <= 0) return;
      s503--;
      const h = Math.floor(s503/3600);
      const m = Math.floor((s503%3600)/60);
      const s = s503 % 60;
      el503.textContent =
    ...
    function subscribeNotif() {
      ...
      btn.innerHTML = '<i class="bi bi-check2-circle"></i> Kami akan kabari!';
