...
<head>
  ...
  <title>404 — Halaman Tidak Ditemukan — {{ config('app.name', 'ResepKita') }}</title>
...
<body>
  ...
  <!-- Blobs -->
  <div class="blob" style="width:420px;height:420px;background:rgba(232,93,38,.08);top:-120px;right:-120px"></div>
  <div class="blob" style="width:300px;height:300px;background:rgba(245,200,66,.07);bottom:-80px;left:-80px"></div>
  ...
  <div class="error-wrap">
    ...
    <!-- Card -->
    <div class="error-card" style="--stripe:linear-gradient(90deg,#ff7b47,#e85d26)">
      <style>.error-card::before{background:linear-gradient(90deg,#ff7b47,#e85d26)}</style>

      <span class="error-emoji">🍽️</span>
      <span class="error-code">404</span>
      <span class="error-pill" style="background:rgba(245,158,11,.1);border:1.5px solid rgba(245,158,11,.28);color:#b45309"><i class="bi bi-exclamation-triangle-fill"></i> Halaman Tidak Ditemukan</span>
      <h1 class="error-title">Halaman Ini Tidak Ada</h1>
      <p class="error-desc">{{ $exception->getMessage() ?: 'Hmm, resep yang kamu cari sepertinya sudah habis atau pindah ke dapur lain. URL yang kamu kunjungi tidak terdaftar di sistem kami.' }}</p>

      
      <div class="error-actions">
        <a href="{{ auth()->check() ? route('dashboard') : url('/') }}" class="btn-primary-e"><i class="bi bi-house-fill"></i> Ke Dashboard</a>
        <a href="{{ url('/') }}" class="btn-outline-e"><i class="bi bi-journal-richtext"></i> Halaman Utama</a>
        <button class="btn-outline-e" onclick="history.back()"><i class="bi bi-arrow-left"></i> Kembali</button>
      </div>
      <div class="error-tips">
        <div class="error-tips-ttl"><i class="bi bi-lightbulb"></i> Mungkin ini yang kamu maksud</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Periksa kembali URL — mungkin ada typo</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Halaman ini mungkin sudah dipindah atau dihapus admin</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Gunakan fitur pencarian di <a href="{{ auth()->check() ? route('dashboard') : url('/') }}" class="tip-link">Dashboard</a> untuk menemukan konten</div>
        <div class="tip-item"><i class="bi bi-check2"></i> Kembali ke halaman sebelumnya dan coba navigasi dari sana</div>
      </div>
